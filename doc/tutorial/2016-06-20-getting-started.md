...
The `route` method method creates an instance of `RouteController` (also a `RequestController` subclass - are you seeing the pattern?). When a router matches the path of an HTTP request to one of its registered routes, the `Request` event is sent to the associated `RouteController`. If no registered route matches the path of the request, the `Router` responds to the `Request` with a 404 status code and drops the `Request` event.
