...
class HTTPBodyDecoder {
  ...
  /// The primary type is the first key in [decoders], and then the subtype is used to return the specific encoding
  ...
  /// The following decoders are supported by default (you may add extra decoders using [addDecoder], do not manipulate [decoders] directly.)
  ///
  ///       application/json
  ///       application/x-www-form-urlencoded
  ///       text/*
  ///
  static Map<String, Map<String, Function>> decoders = {
    "application" : {
      "json" : _jsonDecoder,
  ...
  ///
  /// Associated a [decoder] function for the primary and subtype of [type]. This will replace an existing decoder if one exists.
  /// A [decoder] must return the decoded data as a [Future]. It takes a single [HttpRequest] as an argument. For example, the JSON encoder
  ...
  static void addDecoder(ContentType type, Future<dynamic> decoder(HttpRequest req)) {
    var innerMap = decoders[type.primaryType];
    if (innerMap == null) {
      innerMap = {};
      decoders[type.primaryType] = innerMap;
    }
  ...
  /// for a matching decoding function in [decoders]. If no such decoder is found, the body is returned as a [List] of bytes, where each byte is an [int].
  static Future<dynamic> decode(HttpRequest request) async {
    ...
    var outerMap = decoders[primaryType];
    if (outerMap == null) {
      return _binaryDecoder(request);
    }
  ...
  static BodyStreamDecoder streamDecoderForCharset(String charset, {Encoding defaultEncoding: UTF8}) {
