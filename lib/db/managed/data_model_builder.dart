...
class _DataModelBuilder {
  _DataModelBuilder(ManagedDataModel dataModel, List<Type> instanceTypes) {
    instanceTypes.forEach((type) {
      var entity = new ManagedEntity(dataModel, reflectClass(type), backingMirrorForType(type));
    ...
    entities.forEach((_, entity) {
      ...
      if (entity.primaryKey == null) {
        throw new ManagedDataModelException("No primary key for entity ${MirrorSystem.getName(entity.persistentType.simpleName)}");
      }
    });
  ...
  String tableNameForEntity(ManagedEntity entity) {
  ...
  Map<String, ManagedAttributeDescription> attributeMapForEntity(ManagedEntity entity) {
    ...
    entity.instanceType.declarations.values
      ...
      .fold(<String, ManagedAttributeDescription>{}, (Map<String, ManagedAttributeDescription> collectedMap, attr) {
        if (collectedMap.containsKey(attr.name)) {
          collectedMap[attr.name] = new ManagedAttributeDescription.transient(entity, attr.name, attr.type, managedTransientAttribute);
        } else {
  ...
  ManagedAttributeDescription attributeFromMethodMirror(ManagedEntity entity, MethodMirror methodMirror) {
    ...
    return new ManagedAttributeDescription.transient(entity, name,
        ManagedPropertyDescription.propertyTypeForDartType(dartTypeMirror.reflectedType),
        new ManagedTransientAttribute(availableAsInput: methodMirror.isSetter, availableAsOutput: methodMirror.isGetter));
  }

  ManagedAttributeDescription attributeFromVariableMirror(ManagedEntity entity, VariableMirror mirror) {
    if (entity.instanceType == mirror.owner) {
      // Transient; must be marked as Mappable.

      var name = MirrorSystem.getName(mirror.simpleName);
      var type = ManagedPropertyDescription.propertyTypeForDartType(mirror.type.reflectedType);
      if (type == null) {
        throw new ManagedDataModelException("Property $name on ${MirrorSystem.getName(entity.instanceType.simpleName)} has invalid type");
      }
      return new ManagedAttributeDescription.transient(entity, name, type, _transientFromDeclaration(mirror));
    } else {
      // Persistent
      var attrs = _attributeMetadataFromDeclaration(mirror);

      var type = attrs?.databaseType ?? ManagedPropertyDescription.propertyTypeForDartType(mirror.type.reflectedType);
      if (type == null) {
        throw new ManagedDataModelException("Property ${MirrorSystem.getName(mirror.simpleName)} on ${MirrorSystem.getName(entity.persistentType.simpleName)} has invalid type");
      }

      return new ManagedAttributeDescription(entity, MirrorSystem.getName(mirror.simpleName), type,
  ...
  Map<String, ManagedRelationshipDescription> relationshipMapForEntity(ManagedEntity entity) {
  ...
  ManagedRelationshipDescription relationshipFromVariableMirror(ManagedEntity entity, VariableMirror mirror) {
    if (_attributeMetadataFromDeclaration(mirror) != null) {
      throw new ManagedDataModelException("Relationship ${MirrorSystem.getName(mirror.simpleName)} on ${MirrorSystem.getName(entity.persistentType.simpleName)} must not define additional Attributes");
    }
    ...
    if (belongsToAttr != null) {
      ...
      if (destinationVariableMirror == null) {
        throw new ManagedDataModelException("Relationship ${MirrorSystem.getName(mirror.simpleName)} on ${MirrorSystem.getName(entity.persistentType.simpleName)} has no inverse (tried $inverseKey)");
      }

      if (belongsToAttr.onDelete == ManagedRelationshipDeleteRule.nullify && belongsToAttr.isRequired) {
        throw new ManagedDataModelException("Relationship ${MirrorSystem.getName(mirror.simpleName)} on ${entity.tableName} set to nullify on delete, but is not nullable");
      }

      if (_belongsToMetadataFromDeclaration(destinationVariableMirror) != null) {
        throw new ManagedDataModelException("Relationship ${MirrorSystem.getName(mirror.simpleName)} on ${entity.tableName} and ${MirrorSystem.getName(destinationVariableMirror.simpleName)} on ${destinationEntity.tableName} have BelongsTo metadata, only one may belong to the other.");
      }

      return new ManagedRelationshipDescription(entity, MirrorSystem.getName(mirror.simpleName), referenceProperty.type,
          destinationEntity, belongsToAttr.onDelete, ManagedRelationshipType.belongsTo, inverseKey,
          unique: !(destinationVariableMirror as VariableMirror).type.isSubtypeOf(reflectType(ManagedSet)),
          indexed: true,
    ...
    if (inversePropertyMirror == null) {
      throw new ManagedDataModelException("Relationship ${MirrorSystem.getName(mirror.simpleName)} on ${entity.tableName} has no inverse declared in ${MirrorSystem.getName(destinationEntity.persistentType.simpleName)} of appropriate type.");
    }
  ...
  ManagedEntity destinationEntityForVariableMirror(ManagedEntity entity, VariableMirror mirror) {
    ...
    if (destinationEntity == null) {
      throw new ManagedDataModelException("Relationship ${MirrorSystem.getName(mirror.simpleName)} on ${MirrorSystem.getName(entity.persistentType.simpleName)} destination ModelEntity does not exist");
    }
  ...
  ClassMirror backingMirrorForType(Type instanceType) {
    ...
    if (rt.isSubtypeOf(modelRefl)) {
      ...
      throw new ManagedDataModelException("Invalid instance type $instanceType ${reflectClass(instanceType).simpleName}");
    }
