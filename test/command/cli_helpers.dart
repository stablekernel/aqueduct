...
Future<ProcessResult> runPubGet(Directory workingDirectory, {bool offline: true}) async {
...
Future<CLIResult> runAqueductProcess(List<String> commands, Directory workingDirectory) async {
  commands.add("--directory");
  commands.add("${workingDirectory.path}");

  var result = new CLIResult();
  var cmd = new Runner()..outputSink = result.collectedOutput;
...
Directory getTestProjectDirectory(String name) {
  return new Directory.fromUri(Directory.current.uri.resolve("test/command/migration_test_projects/$name"));
}
