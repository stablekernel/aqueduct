...
void main() {
  ...
  group("Matcher Basics", () {
    ...
    test("Response matcher not using response gives appropriate error", () {
      try {
        ...
        expect(e.toString(), contains("Which: Actual value is not a TestResponse, but instead String."));
      }
    });
    ...
    test("Status code matcher fails with useful message when wrong", () async {
      ...
      try {
        ...
        var str = e.toString();
        expect(str.indexOf("Status Code: 200") > str.indexOf("Status Code: 400"), true);
  ...
  group("Header matchers", () {
    ...
    test("Ensure values of some headers w/ matcher", () async {
      ...
      try {
        expect(response, hasHeaders({
          "x-frame-options" : startsWith("foobar")
        }));
        expect(true, false);
      } on TestFailure catch (e) {
        var string = e.toString();
        expect(string, contains("Headers: x-frame-options: SAMEORIGIN"));
        expect(string, contains("Headers: x-frame-options: a string starting with 'foobar'"));
      }
    });
    ...
    test("Ensure non-existence of header", () async {
      ...
      try {
        expect(response, hasHeaders({
          "x-frame-options" : isNotPresent
        }));
        expect(true, false);
      } on TestFailure catch (e) {
        var string = e.toString();
        expect(string, contains("Expected: Headers: x-frame-options: (Must Not Exist)"));
        expect(string, contains("Headers: x-frame-options: SAMEORIGIN"));
        expect(string, contains("Which: x-frame-options must not be present, but was SAMEORIGIN"));
      }
    });
    ...
    test("Ensure any headers other than those specified", () async {
      ...
      try {
        ...
        var string = e.toString();
        expect(string, contains("Header content-type was in response headers, but not part of the match set and failIfContainsUnmatchedHeader was true"));
        expect(string, contains("Header x-xss-protection was in response headers, but not part of the match set and failIfContainsUnmatchedHeader was true"));
        expect(string, contains("Header x-content-type-options was in response headers, but not part of the match set and failIfContainsUnmatchedHeader was true"));
        expect(string, contains("Header content-length was in response headers, but not part of the match set and failIfContainsUnmatchedHeader was true"));
      }
    });
  });
  ...
  group("Body, content-type matchers", () {
    ...
    test("Can match empty body", () async {
      ...
      server.queueResponse(new Response.ok(null, headers: {"Content-Type" : "application/json"}));
    ...
    test("Can match JSON Object", () async {
      ...
      try {
        ...
        expect(e.toString(), contains("Expected: Body: contains pair 'nocontenttype' => 'thatsaysthisisjson'"));
        expect(e.toString(), contains("Body: {nocontenttype: thatsaysthisisjson}"));
      }
    });
  });
  ...
  group("Body, value matchers", () {
    ...
    test("List of terms", () async {
      ...
      try {
        ...
        expect(e.toString(), contains("Expected: Body: every element(a value less than <0>)"));
        expect(e.toString(), contains("Body: [1,2,3]"));
      }
    });
    ...
    test("Exact map", () async {
      ...
      try {
        ...
        expect(e.toString(), contains("Expected: Body: {'foo': 'notbar', 'x': 'y'}"));
    ...
    test("Map with matchers", () async {
      ...
      try {
        ...
        expect(e.toString(), contains("Expected: Body: {'foo': <not an instance of String>, 'x': 5}"));
    ...
    test("Partial match, one level", () async {
      ...
      try {
        ...
        expect(e.toString(), contains("Expected: Body: Partially matches: {foo: 'notbar',}"));
        expect(e.toString(), contains('Body: {"foo":"bar","x":5}'));
      }
      try {
        ...
        expect(e.toString(), contains("Expected: Body: Partially matches: {x: a value less than <0>,}"));
        expect(e.toString(), contains('Body: {"foo":"bar","x":5}'));
      }
    });
  });
