...
void main() {
  ...
  group("Create table", () {
    ...
    test("Create table with unique constraints", () {
      builder.createTable(SchemaTable("foo", [SchemaColumn("id", ManagedPropertyType.integer), SchemaColumn("x", ManagedPropertyType.integer)], uniqueColumnSetNames: ["id", "x"]));
  ...
  group("Alter table", () {
    test("Alter table; change unique", () {
      builder.createTable(SchemaTable("foo", [SchemaColumn("id", ManagedPropertyType.integer), SchemaColumn("x", ManagedPropertyType.integer), SchemaColumn("y", ManagedPropertyType.integer)], uniqueColumnSetNames: ["id", "x"]));
    ...
    test("Alter table; add unique", () {
      builder.createTable(SchemaTable("foo", [SchemaColumn("id", ManagedPropertyType.integer), SchemaColumn("x", ManagedPropertyType.integer), SchemaColumn("y", ManagedPropertyType.integer)]));
      builder.alterTable("foo", (t) {
        t.uniqueColumnSet = ["x", "y"];
      });
      expect(builder.commands.length, 2);
  ...
  group("Add column", () {
    ...
    test("Add relationship column", () {
      ...
      builder.addColumn("bar", SchemaColumn.relationship("foo_id", ManagedPropertyType.integer, relatedTableName: "foo", relatedColumnName: "id"));
      expect(builder.commands.length, 3);
      expect(builder.commands.last, "database.addColumn(\"bar\", "
  ...
  group("Alter column", () {
    ...
    test("isNullable foreign key", () {
      ...
      builder.createTable(SchemaTable("bar", [
        ...
        SchemaColumn.relationship("foo_id", ManagedPropertyType.integer, isNullable: false, relatedTableName: "foo", relatedColumnName: "id")
      ]));
    ...
    test("deleteRule", () {
      ...
      builder.createTable(SchemaTable("bar", [
        SchemaColumn("id", ManagedPropertyType.integer),
        SchemaColumn.relationship("foo_id", ManagedPropertyType.integer, relatedTableName: "foo", relatedColumnName: "id", rule: DeleteRule.cascade)
      ]));
      builder.alterColumn("bar", "foo_id", (c) {
        c.deleteRule = DeleteRule.nullify;
      });
      expect(builder.commands.length, 3);
      expect(builder.commands.last, "database.alterColumn(\"bar\", \"foo_id\", (c) {c.deleteRule = DeleteRule.nullify;});");

      builder.alterColumn("bar", "foo_id", (c) {
        c.deleteRule = DeleteRule.cascade;
      });
      expect(builder.commands.length, 4);
      expect(builder.commands.last, "database.alterColumn(\"bar\", \"foo_id\", (c) {c.deleteRule = DeleteRule.cascade;});");
    ...
    test("Multiple statements", () {
      builder.createTable(SchemaTable("foo", [SchemaColumn("id", ManagedPropertyType.integer, isIndexed: false, isUnique: false)]));
      builder.alterColumn("foo", "id", (c) {
        c.isIndexed = true;
        c.isUnique = true;
      });
      expect(builder.commands.length, 2);
      expect(builder.commands.last, "database.alterColumn(\"foo\", \"id\", (c) {c.isIndexed = true;c.isUnique = true;});");
    });
  });
}
