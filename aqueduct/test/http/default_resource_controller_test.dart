  ...
  group("Extended GET requests", () {
    ...
    test("Can get all objects w/ count and offset", () async {
      expect(
          ...
          hasResponse(
              200, body: allObjects.sublist(1, 3).map((m) => m.asMap()).toList()));
    });
    ...
    test(
      ...
      expect(
          ...
          hasResponse(400,
            body: {"error": "cannot sort by '[foobar,asc]'"}));
    });
    ...
    test("Paging after", () async {
      expect(
          await client
              ...
              .get(),
          hasResponse(
              200, body: allObjects.sublist(6).map((m) => m.asMap()).toList()));
    });
    ...
    test("Paging with wrong key", () async {
      expect(
          ...
          hasResponse(400,
            body: {"error": "cannot page by 'foobar'"}));
    });
  });
