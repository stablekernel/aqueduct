...
void main() {
  ...
  group("Refreshing token", () {
    ...
    test("Can refresh token if client has redirect uri", () async {
      ...
      var refreshToken = await auth!
          .refresh(token.refreshToken, "com.stablekernel.redirect", "mckinley");
  ...
  group("Cleanup/friendly fire scenarios from client", () {
    ...
    test("Revoking a client revokes all of its tokens and auth codes",
      ...
      try {
        await auth!
            .exchange(unusedCode.code, "com.stablekernel.redirect", "mckinley");
        expect(true, false);
        // ignore: empty_catches
      } on AuthServerException {}
    ...
    test(
      ...
      await auth!.authenticate(createdUsers!.first.username,
          User.defaultPassword, "com.stablekernel.redirect", "mckinley");
      ...
      var issuedTokenKeep = await auth!.authenticate(
          createdUsers!.first.username,
          User.defaultPassword,
          "com.stablekernel.redirect2",
          "gibraltar");
  ...
  group("Code friendly fire/cleanup", () {
    ...
    test(
      ...
      var newCode = await auth!.authenticateForCode(
          createdUsers!.first.username,
          User.defaultPassword,
          "com.stablekernel.redirect");
      var codeQuery = Query<ManagedAuthToken>(context);
      ...
      var lastCode = await auth!.authenticateForCode(
          createdUsers!.first.username,
          User.defaultPassword,
          "com.stablekernel.redirect2");
    ...
    test("Client that requests allowed nested scope gets token", () async {
      ...
      try {
        var _ = await auth!.authenticate(createdUser!.username,
            User.defaultPassword, "subset.multiple", null,
            requestedScopes: [
              AuthScope("user"),
            ]);
        expect(true, false);
