...
void main() {
  ...
  group("Comment Documentation", () {
    ...
    test("If entity has comment documentation, it is in schema description", () {
    ...
    test("Entity has notes even if it doesn't have comments", () {
      ...
      expect(doc.components.schemas["Model3"].description,
          contains("No two objects may have the same value for all of: 'matches', 'lessThan'."));
    });

    test(
        "Title and description contain docs, even if no docs added by auto-generated docs",
        () {
      expect(schema.properties["getter"].title, "getter");
      expect(schema.properties["getter"].description, contains("description"));
      expect(schema.properties["getter"].description, contains("title"));
    });

    test(
        "Title and description are pre-pended to docs auto-generate for property",
        () {
      expect(schema.properties["string"].title, "string");
      expect(schema.properties["string"].description, contains("title"));
    ...
    test("No autogenerated doc comments yields empty property description",
        () {
      expect(schema.properties["dateTime"].title, "dateTime");
      expect(schema.properties["dateTime"].description, "");
    });

    test("Relationship can have doc comments", () {
      expect(schema.properties["model2s"].title, "model2s");
      expect(schema.properties["model2s"].description, contains("description"));
      expect(schema.properties["model2s"].description, contains("title"));
    });
  });
