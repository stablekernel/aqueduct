...
class DataModelBuilder {
  DataModelBuilder(ManagedDataModel dataModel, List<Type> instanceTypes) {
    ...
    _builders.forEach((b) => b.linkBuilders(_builders.where((i) => i != b).toList()));
    _validate();

    final entityList = _builders.map((eb) => eb.getEntity(dataModel)).toList();
    _builders.forEach((b) {
      final entity = b.getEntity(dataModel);
      b.linkEntities(dataModel, entityList.where((me) => me != entity).toList());
  ...
  void _validate() {
    final missingInverseErrorBuffer = <String>[];
    _builders.forEach((b) {
      final missingInverses = b.properties.where((pb) => pb.isRelationship && pb.relatedProperty == null).toList();
      if (missingInverses.isNotEmpty) {
        missingInverseErrorBuffer.addAll(missingInverses.map((p) => "${p.entity.name}.${p.name}"));
      }
    });
    if (missingInverseErrorBuffer.isNotEmpty) {
      throw ManagedDataModelError("The inverse for the following "
        "relationship properties could not be found: ${missingInverseErrorBuffer.join(", ")}");
    }
